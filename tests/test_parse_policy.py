...
def test_execution_mode_is_loaded_from_environment(
    ...
    """Execution mode env var is honored when API override is absent."""
    configuration.reset_api_overrides()
...
def test_api_override_has_precedence_over_environment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """API-set execution mode is not replaced by environment settings."""
    monkeypatch.setenv("MUNINN_PARSER_EXECUTION_MODE", "centralized_first_fallback")
    set_execution_mode(ExecutionMode.LOCAL_FIRST_FALLBACK)
...
def test_local_only_with_no_candidates_raises_parser_not_found() -> None:
    """Unknown command still raises parser-not-found in local-only mode."""
    set_execution_mode(ExecutionMode.LOCAL_ONLY)
    with pytest.raises(ParserNotFoundError):
        parse("nxos", "show ip route", "output")
