"""Parser for 'show bgp all evpn summary' command on Cisco IOS-XR.

IOS-XR ``show bgp all evpn summary`` displays BGP process information
and the neighbor table for the EVPN address family.  The output format
is identical to ``show bgp summary`` (single-AF variant); parsing logic
...
__all__ = ["ShowBgpAllEvpnSummaryParser"]


@register(OS.CISCO_IOSXR, "show bgp all evpn summary")
class ShowBgpAllEvpnSummaryParser(BaseParser["ShowBgpSummaryResult"]):
    """Parser for 'show bgp all evpn summary' on Cisco IOS-XR.

    Parses BGP process information, speaker table, and neighbor summary
    table for the EVPN address family.  Supports IPv6 neighbor address
    wrapping.
    ...
    def parse(cls, output: str) -> ShowBgpSummaryResult:
        """Parse 'show bgp all evpn summary' output on Cisco IOS-XR.
