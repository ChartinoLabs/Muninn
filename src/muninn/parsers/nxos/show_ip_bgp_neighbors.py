...
class ShowIpBgpNeighborsResult(TypedDict):
    """Schema for 'show ip bgp neighbors' parsed output on NX-OS."""
...
_NEIGHBOR_HEADER_RE = re.compile(
    ...
    r"(?P<link_type>\S+) link,\s+"
    r"Peer index (?P<peer_index>\d+)"
)

_PEER_TEMPLATE_RE = re.compile(
    r"^\s+Inherits peer configuration from peer-template "
    r"(?P<template>\S+)"
)
...
_MD5_AUTH_RE = re.compile(
    r"^\s+TCP MD5 authentication is "
    r"(?:set \((?P<state>\w+)\)|(?P<state2>\w+))"
)
...
_AF_ACCEPTED_PATHS_RE = re.compile(
    r"^\s+(?P<count>\d+) accepted paths consume "
    r"(?P<memory>\d+) bytes"
)

_AF_SENT_PATHS_RE = re.compile(r"^\s+(?P<count>\d+) sent paths")
...
def _try_parse_transport_fields(line: str, entry: NeighborEntry) -> bool:
    ...
    if m:
        entry["md5_auth"] = m.group("state") or m.group("state2")
        return True

    m = _BFD_RE.match(line)
    if m:
